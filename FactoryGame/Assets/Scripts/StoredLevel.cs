...
public class StoredLevel
{
    ...
    public StoredLevel(string fileName, bool shouldBeEmpty = false)
	{
		...
		foreach (Transform child in Builder.Singleton.buildableParent)
		{
	...
	public void ToLevel()
	{
		...
		Builder.Singleton.ClearLevel();

		foreach (StoredBuildable storedBuildable in buildables)
		{
			Buildable buildable = Builder.Singleton.GetBuildableByUUID(storedBuildable.UUID);
			if (buildable == null)
				continue;
			...
			BuildProperties properties = new BuildProperties(rotation, position, buildable.UUID);

			Builder.Singleton.BuildServerRpc(properties, true);
